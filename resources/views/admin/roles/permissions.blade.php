...
@section('page-title', 'Manage Permissions: ' . $role->display_name)
                ...
                <div class="card-header bg-white py-4 px-4 border-bottom-0">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div>
                                <h5 class="mb-0 fw-bold text-dark">Permissions: {{ $role->display_name }}</h5>
                                <p class="text-muted small mb-0">Manage access levels for this role</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card-body px-4 pb-4">
                    <form action="{{ route('admin.roles.permissions.update', $role->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <div class="d-flex justify-content-between align-items-center mb-4 ps-1">
                                <h6 class="fw-bold mb-0">
                                    <i class="bi bi-ui-checks me-2 text-primary"></i>Assigned Permissions
                                </h6>
                            </div>
                                        ...
                                        <div class="permission-list-horizontal">
                                            @foreach($groupPermissions as $permission)
                                                ...
                                                <div class="permission-card bg-white rounded-3 px-3 py-2.5 d-flex align-items-center justify-content-between shadow-sm" 
                                                     @if($role->name !== 'super_admin') onclick="toggleCheckbox(this)" @endif>
                                                    <div class="d-flex align-items-center flex-grow-1">
                                                        <input class="form-check-input my-0 me-3 flex-shrink-0" type="checkbox" 
                                                            name="permissions[]" 
                                                            value="{{ $permission->id }}" 
                                                            id="perm_{{ $permission->id }}"
                                                            {{ $role->name === 'super_admin' ? 'checked disabled' : ($role->permissions->contains($permission->id) ? 'checked' : '') }}>
                        ...
                        <div class="d-flex justify-content-between align-items-center pt-3">
                            ...
                            @else
                                <button type="button" class="btn btn-success px-5 py-2 fw-semibold rounded-pill shadow-sm" disabled>
...
<script>
    function toggleCheckbox(element) {
        // Prevent if element is disabled or read-only/Super Admin
        @if($role->name === 'super_admin') return; @endif
    ...
    function toggleGroupPermissions(btn) {
        ...
        allCheckboxes.forEach(cb => {
            if (!cb.disabled) cb.checked = newState;
        });

...
@endsection
    ...
    @section('content')
        <div class="row justify-content-center"><div class="col-12"><div class="card border-0 shadow-lg rounded-4"><div class="card-header bg-white py-4 px-4 border-bottom-0"><div class="d-flex justify-content-between align-items-center"><div class="d-flex align-items-center gap-3"><div class="bg-warning bg-opacity-10 text-warning rounded-circle p-2 d-flex align-items-center justify-content-center" style="width: 40px; height: 40px;"><i class="bi bi-pencil-square fs-5"></i></div><div><h5 class="mb-0 fw-bold text-dark">Edit Role:
        {{ $role->display_name }}
        </h5><p class="text-muted small mb-0">Modify role details and permissions</p></div></div></div></div><div class="card-body px-4 pb-4"><form action="{{ route('admin.roles.update', $role->id) }}" method="POST">@csrf
                        @method('PUT')
                        <div class="row g-4">
                            <div class="col-md-6">
                                <div class="mb-2">
                                    ...
                                    <input type="text" class="form-control form-control-lg @error('name') is-invalid @enderror" id="name"
                                        name="name" value="{{ old('name', $role->name === 'super_admin' ? $role->display_name : $role->name) }}" required>
                                    @error('name')
                            ...
                            <div class="col-md-6">
                                <div class="mb-2">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" 
                                        id="description" name="description" style="height: 60px" required>{{ old('description', $role->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            </div>
                        </div><hr class="my-4 border-light"><div class="mb-4"><div class="d-flex justify-content-between align-items-center mb-4 ps-1"><h6 class="fw-bold mb-0"><i class="bi bi-ui-checks me-2 text-primary"></i>Assign Permissions                            <div class="permission-grid">
                                ...
                                @endforelse
                            </div></div><div class="d-flex justify-content-between align-items-center pt-3"><a href="{{ route('admin.roles.index') }}" class="btn btn-light text-secondary fw-medium px-4"><i class="bi bi-arrow-left me-2"></i>Cancel </a><button type="submit" class="btn btn-primary px-5 py-2 fw-semibold rounded-pill shadow-sm"><i class="bi bi-check-lg me-2"></i>Update Role </button></div></form></div></div></div></div>@endsection

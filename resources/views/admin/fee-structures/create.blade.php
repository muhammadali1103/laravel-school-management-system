...
@section('content')
    <div class="card">
        <div class="card-header bg-white">
            <h5 class="mb-0">Create New Fee Structure</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.fee-structures.store') }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                name="name" value="{{ old('name') }}" placeholder="e.g., Grade 9 Annual Tuition Fee" required>
                            @error('name')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('fee_type') is-invalid @enderror" id="fee_type"
                                ...
                                <option value="Admission Fee" {{ old('fee_type') == 'Admission Fee' ? 'selected' : '' }}>Admission Fee</option>
                                <option value="Transport Fee" {{ old('fee_type') == 'Transport Fee' ? 'selected' : '' }}>Transport Fee</option>
                                <option value="Laboratory Fee" {{ old('fee_type') == 'Laboratory Fee' ? 'selected' : '' }}>Laboratory Fee</option>
                                <option value="Library Fee" {{ old('fee_type') == 'Library Fee' ? 'selected' : '' }}>Library Fee</option>
                                <option value="Sports Fee" {{ old('fee_type') == 'Sports Fee' ? 'selected' : '' }}>Sports Fee</option>
                                <option value="Examination Fee" {{ old('fee_type') == 'Examination Fee' ? 'selected' : '' }}>Examination Fee</option>
                                <option value="Annual Fee" {{ old('fee_type') == 'Annual Fee' ? 'selected' : '' }}>Annual Fee</option>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label for="Amount (PKR)" class="form-label">Amount (PKR) <span class="text-danger">*</span></label>
                            <input type="number" step="0.01" class="form-control @error('Amount (PKR)') is-invalid @enderror"
                                id="Amount (PKR)" name="Amount (PKR)" value="{{ old('Amount (PKR)') }}" required>
                            @error('Amount (PKR)')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
